...
```java
@SpringBootApplication(scanBasePackages = {"com.yourproject", "com.externalpackage"})
```
...
If we don't want : @SpringBootApplication(exclude = {DataSourceAutoConfiguration.class})
...
1. [ Tomcat Server ]       <-- Receives raw network traffic on port 8080
   │
   ▼
2. [ DispatcherServlet ]   <-- Hosted by Tomcat; figures out which Spring Controller to call
   │
   ▼
3. [ Your @RestController ] <-- Your custom business logic executes
```
...
What exactly does Application Context contain?
...
injects PaymentService into it
keeps both as managed beans
